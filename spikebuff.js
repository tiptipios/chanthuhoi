   function Camcao() {
       ...
       function ZomCamxa() {
    modifyValue7(60, 99, '68');
}
function modifyValue7(searchValue, replaceValue, addressEndsWith) {
    h5gg.clearResults();
    ...
    results.forEach(result => {
        if (result.address.endsWith(addressEndsWith)) {
            h5gg.setValue(result.address, replaceValue, 'F32');
            lockAddresses.push(result.address);
        }
    });
        ...
        const locker = setInterval(() => {
            lockAddresses.forEach(address => {
                ...
                if (currentValue !== replaceValue) {
                    h5gg.setValue(address, replaceValue, 'F32');
                }
            });
        }, 1000); // Kiểm tra và cập nhật mỗi 2 giây
    }
}
function tipAimCope() {
    function searchAndReplace(searchValue, replaceValue, type) {
        ...
        for (var i = 0; i < results.length; i++) {
            h5gg.setValue(results[i].address, replaceValue, type);
        }
    }
    // Chuyển đổi giá trị thành số nguyên nếu kiểu là 'I64'
    searchAndReplace(0.09, 180, 'F32');
}
                function tiphsCu() {
                    modifyValue(96688289, 2018908708, '4');
                }
    ...
    modifyValue1(5.5,99999999999,'16C');
    modifyValue1(0.75,99999999999,'174');
                }
    ...
    for (var i = 0; i < count; i++) {
        ...
        var tiptop3 = (Number(addr1) + -0);
        {
            ...
            h5gg.setValue(tiptop3, hk1, "F32");
                ...
                function tipAim180() {
                    modifyValue(Buoc1, -2000, 'A44','9C4'); 
                    modifyValue(Buoc2, -2000, 'A3C','9BC'); 
                    modifyValue(Buoc3, -2000, '9B0', '9B8');
                    modifyValue(1057048494, -2000, 'A30', 'A38')}
    
                function tipAimFov() {
                    modifyValue(1077936128, -20000, 'A04','984');
                    modifyValue(1069547520, 1148829696, 'A10','990')}
